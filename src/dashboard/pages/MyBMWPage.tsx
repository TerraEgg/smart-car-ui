...
export const MyBMWPage: React.FC<MyBMWPageProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (state.speed > 0) {
        setDistance((prev) => prev + (state.speed / 100) * 0.016);
      }
      const rpmValue = Math.round((state.speed / 200) * 6000);
      setRPM(rpmValue);
      if (state.speed > 0) {
  ...
  const handleMouseUp = () => {
    ...
    const distances = {
      ...
      "top-right": Math.sqrt(
        (dragPosition.x - (rect.width - panelWidth)) ** 2 + dragPosition.y ** 2
      ),
      "bottom-left": Math.sqrt(
        dragPosition.x ** 2 +
          (dragPosition.y - (rect.height - panelHeight)) ** 2
      ),
      "bottom-right": Math.sqrt(
        (dragPosition.x - (rect.width - panelWidth)) ** 2 +
          (dragPosition.y - (rect.height - panelHeight)) ** 2
      ),
    };
    ...
    if (nearestCorner === "top-right") {
      ...
      newPos = {
        x: rect.width - panelWidth - padding,
        y: rect.height - panelHeight - padding,
      };
    }
      ...
      >
        ...
        <div
          style={{
            ...
            width: "320px",
            background:
              "linear-gradient(135deg, rgba(255, 255, 255, 0.95) 0%, rgba(242, 210, 220, 0.1) 100%)",
            borderRadius: "20px",
            padding: "24px",
            boxShadow:
              "0 12px 40px rgba(0, 0, 0, 0.12), inset 0 1px 1px rgba(255, 255, 255, 0.4)",
