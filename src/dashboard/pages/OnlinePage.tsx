...
import { Search, Play, Pause, Volume2, VolumeX, Loader, ArrowLeft, Music, X, Upload } from 'lucide-react';
...
export const OnlinePage: React.FC<OnlinePageProps> = ({ 
  ...
  const currentTrack = sharedCurrentTrack !== undefined ? sharedCurrentTrack : localCurrentTrack;
  ...
  // Load local tracks from localStorage on mount
  useEffect(() => {
  ...
  const playTrack = (track: Track) => {
    if (onTrackPlay) {
      // Use Dashboard's audio management
      onTrackPlay(track);
    } else {
  ...
  const handleBack = () => {
    setIsFadingOut(true);
    // Don't stop audio when going back - let it continue playing
    setTimeout(() => {
      onBack();
  ...
  return (
    ...
    >
      {/* Local audio element only if not using shared ref */}
      {!sharedAudioRef && <audio ref={audioRef} onEnded={() => setLocalIsPlaying(false)} crossOrigin="anonymous" />}
      ...
      {currentTrack && (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', minWidth: '70px' }}>
            {volume > 0 ? <Volume2 size={14} color="#6b4a5a" /> : <VolumeX size={14} color="#6b4a5a" />}
            <input
              type="range"
              min="0"
              max="100"
      ...
      >
        ...
        >
          {isSearching ? (
            <Loader size={18} color="#6b4a5a" style={{ marginRight: '8px', animation: 'spin 1s linear infinite' }} />
          ) : (
